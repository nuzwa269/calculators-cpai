...
export function CalcLayout({
  ...
  return (
    <div className="min-h-screen bg-background">
      <SiteHeader />
      <main className="bg-[#0b1220] pb-16">
        {/* Hero split: form (left, white) + teaser (right, gradient blue) */}
        <section className="relative">
          <div className="mx-auto max-w-6xl px-4 pt-8 sm:px-6 sm:pt-12">
            <a
              href="/"
              className="mb-4 inline-flex items-center text-xs font-medium text-slate-400 transition-colors hover:text-white"
            >
              ← All calculators
            </a>
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]">
              {/* Left: form card */}
              <article className="relative overflow-hidden rounded-3xl bg-white p-6 text-slate-900 shadow-2xl sm:p-8">
                <span className="inline-flex items-center rounded-full bg-indigo-50 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-indigo-700 ring-1 ring-inset ring-indigo-100">
                  {content?.eyebrow ?? "FREE CALCULATOR"}
                </span>
                <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-slate-900 sm:text-[2.25rem] sm:leading-[1.1]">
                  {headline}
                </h1>
                <p className="mt-3 max-w-xl text-sm text-slate-600 sm:text-[15px] sm:leading-relaxed">
                  {hook}
                </p>
                ...
                <div className="mt-8 [&_label]:text-slate-700 [&_input]:bg-white [&_input]:text-slate-900 [&_[data-slot=select-trigger]]:bg-white [&_[data-slot=select-trigger]]:text-slate-900">
                  {children}
                </div>
              </article>
              ...
              {/* Right: teaser card */}
              {content?.teaser && (
                <aside className="relative hidden overflow-hidden rounded-3xl p-6 shadow-2xl sm:p-8 lg:block bg-gradient-to-br from-sky-400 via-blue-500 to-indigo-600 text-white">
                  <div
                    aria-hidden
                  ...
                  <div
                    aria-hidden
                    className="absolute -bottom-24 -left-10 h-64 w-64 rounded-full bg-white/10 blur-2xl"
                  />
                  <div className="relative">
                    <span className="inline-flex items-center rounded-full bg-white/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-white ring-1 ring-inset ring-white/25">
                      {content.teaser.eyebrow}
                    </span>
                    <h2 className="mt-4 text-2xl font-extrabold leading-tight">
                      {content.teaser.title}
                    </h2>
                    <p className="mt-2 text-sm text-white/90">{content.teaser.body}</p>
                    ...
                    <div className="mt-4 rounded-2xl bg-slate-900/85 p-4 text-white">
                    ...
                    <button
                      type="button"
                      className="mt-4 w-full rounded-2xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-lg transition-colors hover:bg-slate-800"
                    >
        ...
        <div className="mx-auto mt-10 max-w-6xl space-y-6 px-4 sm:px-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-3">
                {related.map((r) => (
                  <a
                    key={r.slug}
                    href={`/${r.slug}`}
                    className="group rounded-2xl border border-slate-200 bg-white p-5 transition-all hover:-translate-y-0.5 hover:border-indigo-500 hover:shadow-lg"
                  >
          ...
          {content?.cta && (
            <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 to-indigo-950 p-6 text-white shadow-2xl sm:p-10">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <button
                  ...
                  className="inline-flex items-center rounded-xl bg-amber-400 px-5 py-2.5 text-sm font-bold text-slate-900 shadow-lg transition-colors hover:bg-amber-300"
                >
                  {content.cta.primary}
                </button>
                <a
                  href="/"
                  className="inline-flex items-center rounded-xl border border-white/20 bg-white/5 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/10"
                >
          ...
          {content?.disclaimer && (
            <p className="pt-4 text-xs leading-relaxed text-slate-500">{content.disclaimer}</p>
          )}
        </div>
      </main>
      <SiteFooter />
